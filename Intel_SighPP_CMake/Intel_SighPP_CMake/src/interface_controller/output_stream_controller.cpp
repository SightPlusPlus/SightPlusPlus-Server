...
void OutputStreamController::output_to_color_window(cv::Mat color_matrix, cv::Mat depth_matrix, const std::vector<PrioritisedClassificationResult>& vector) const
{
	for (auto&& prioritised_classification_result : vector)
	{
		for (auto&& item : prioritised_classification_result.objects)
		{
			...
			auto base_line = 0;
			auto label_size = cv::getTextSize(ss.str(), cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &base_line);
			...
			putText(color_matrix, ss.str(), center, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
		}
	}
